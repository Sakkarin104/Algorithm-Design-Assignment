    ...
    public static void main(String[] args) {
        ...
        for (int n : sizes) {
            ...
            for (int r = 0; r < runs; r++) {
                // วัดเวลา Iterative
                long startIter = System.nanoTime(); //[cite: 1]
                reverseIterative(data);
                long endIter = System.nanoTime(); //[cite: 1]
                totalIterative += (endIter - startIter);

                // วัดเวลา Recursive (ใส่ try-catch ป้องกัน StackOverflow)
                try {
                    long startRec = System.nanoTime(); //[cite: 1]
                    reverseRecursive(data);
                    long endRec = System.nanoTime(); //[cite: 1]
        ...
        for (int n : sizes) {
            // เตรียมข้อมูลทดสอบ (อาร์เรย์เรียงลำดับ)
            int[] data = new int[n];
